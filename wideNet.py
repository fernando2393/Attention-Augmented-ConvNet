...
def wide_basic(inputs, in_planes, out_planes, dropout_rate, stride, name, index):
    with tf.variable_scope('wide_basic' + name):
        ...
        x = tf.contrib.layers.batch_norm(inputs, updates_collections=None, decay=0.9, scale=True, center=True,
                                         is_training=istraining_ph)
        x = tf.nn.relu(x)
        ...
        x = tf.contrib.layers.batch_norm(x, updates_collections=None, decay=0.9, scale=True, center=True,
                                         is_training=istraining_ph)
        x = tf.nn.relu(x)
...
loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits_train, labels=y_tf)) + 5E-4 * tf.add_n(
    [tf.nn.l2_loss(v) for v in var_filt])
